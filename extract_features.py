...
def main(args):
    ...
    if args.vae == 'ours':
        ...
        from ldm.models.autoencoder import AutoencoderKL, AutoencoderKLWrapper
        ...
        def load_kl(config, ckpt_path=None, vae_type="sd"):

            if vae_type == "sd":
                model = AutoencoderKL(**config.model.params)
            else:
                model = AutoencoderKLWrapper(**config.model.params)
        ...
        vae = load_kl(vae_config, ckpt_path=args.vae_ckpt, vae_type=args.vae_type).to(device)
    ...
    for x, y in tqdm(loader):
        ...
        with torch.no_grad():
            ...
            if args.vae == "ours":
                ...
                # x  = vae.encode(x).sample().mul_(0.1045)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--vae", type=str, choices=["ema", "mse", "xl", "sd3","ours"], default="ema")  # Choice doesn't affect training
    parser.add_argument("--vae-ckpt", type=str, default=None)  # Choice doesn't affect training
    parser.add_argument("--vae-config", type=str, default=None)  # Choice doesn't affect training
    parser.add_argument("--vae-type", type=str, default=None)  # Choice doesn't affect training

    parser.add_argument("--vae-scaling-factor", type=float, default=0.18215)  # Choice doesn't affect training
