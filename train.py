...
def main(args):
    ...
    if args.multi_res:
        experiment_dir += "-multi-res"
        if args.interpolation:
            experiment_dir += f"-interpolation-{args.interpolation}"

        if args.before_unpatchify:
            experiment_dir += f"-before_unpatchify"

        if args.registers:
            experiment_dir += f"-registers"

    if args.one_per_res:
        experiment_dir += f"-1_per_res"

        if args.registers:
            experiment_dir += f"-registers"
            
    if args.wavelets:
        experiment_dir += f"-wavelets"



    if args.laplacian:
        experiment_dir += f"-laplacian"

    if args.diff_proj:
        experiment_dir += f"-diff_proj"

    if args.gaussian_registers:
        experiment_dir += f"-gaussian_registers"

    if args.x0_pred:
        experiment_dir += f"-x0_pred"
    ...
    model = DiT_models[args.model](
        ...
        learn_sigma=False if args.x0_pred else True,
...
if __name__ == "__main__":
    ...
    parser.add_argument("--interpolation", type=str, default="nearest", choices=["bicubic", "nearest"])
    ...
    parser.add_argument("--x0-pred", type=str, default=False)
